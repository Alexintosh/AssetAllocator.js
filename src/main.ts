...
const multiplyMatricies = (a, b) => {
    const matrixShape = new Array(a.length).fill(0)
      .map(() => new Array(b[0].length).fill(0));
        return matrixShape.map((row, i) =>
          row.map((_val, j) => getDot(a, b, i, j)));
      }
...
export class IndexCalculator {
    ...
    async fetchCoinData(id) {
        const res = await fetch(`https://api.coingecko.com/api/v3/coins/${id}/market_chart?days=30&vs_currency=usd&interval=daily`);
    ...
    async pullData(useSnapshot=false, tokens) {
        for (const token of tokens) {
            ...
            const response: any = await this.fetchCoinData(token.coingeckoId);
    ...
    computeMCAP() {
        this.dataSet.forEach(el => {
            ...
            const marketCap = el.data.market_caps.map( o => {
                // o[0] Timestamp
                // o[1] Value
                return o[1];
            })
    ...
    getTotal() {
        const total = this.dataSet.reduce((a, v) => a+v.RATIO, 0 );
        console.log(`\nTotal: ${round(total)}\n`)
        return total;
    }
    ...
    computeBacktesting() {
        this.dataSet.forEach(el => {
            ...
            for (let i = 0; i < el.data.prices.length; i++) {
                ...
                if( i === 0) {
                    ...
                    const prePrice = el.data.prices[i-1][1]
                    const ln = Math.log( price /  prePrice)
        ...
        this.dataSet.forEach(el => {
            const logs = el.data.prices.map( o => {
                return o[2];
            });
    ...
    computeCorrelation() {
        for (let i = 0; i < this.dataSet.length; i++) {
            ...
            for (let k = 0; k < this.dataSet.length; k++) {
                ...
                const correlation = jStat.corrcoeff(current.backtesting.returns, next.backtesting.returns);
    ...
    computeCovariance() {
        ...
        for (let i = 0; i < this.dataSet.length; i++) {
            ...
            for (let k = 0; k < this.dataSet.length; k++) {
                const next = this.dataSet[k];
                const covariance = jStat.covariance(current.backtesting.returns, next.backtesting.returns) * current.backtesting.returns.length;
        ...
        const weightsArray = this.dataSet.map( el => el.RATIO);
        weightsArray.forEach(el => matrixC.push([el]));

        const product = multiplyMatricies([ weightsArray ] , matrixB);
    ...
    computeMCTR() {
        ...
        for (let i = 0; i < this.dataSet.length; i++) {
            ...
            for (let k = 0; k < this.dataSet.length; k++) {
                ...
                const x = next.RATIO * current.STDEV * next.STDEV * current.backtesting.correlation[next.name];
                tempCalc += x;
            }
